...
// 文字の分類（句読点・縦書きで回す文字・かっこ）は
// 解析ロジックと同じものを使う必要があるため ./parseText から取り込んでいる。
...
const PreviewArea = ({ state, updateState, isGenko, isLandscape, scrollRef }) => {
  ...
  const clearDecoration = () => {
    ...
    if (beforeState.border) closingTags += BORDER_CLOSE[beforeState.border];
    ...
    if (afterState.border) reopeningTags += BORDER_OPEN[afterState.border];
...
// --- テキスト解析（本体は ./parseText.js。ここは React のキャッシュを挟むだけ） ---
function useParsedText(text, cellsPerLine, maxLines, direction) {
  return useMemo(
    () => parseTextToPages(text, cellsPerLine, maxLines, direction),
