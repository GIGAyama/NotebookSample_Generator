...
| 9 | 最大ファイルサイズ | ⚠️ | ✅ | 解析ロジックを `src/parseText.js` に分離（App.jsx 1,673行 → 1,519行） |
| — | MANUAL.md | ❌ | ✅ | 先生向け・専門用語ゼロ。「うまくいかないとき」の節あり |
| — | 品質ゲート（§4） | ❌ | ✅ | 正本を移植し CI に組み込み。0 error |
...
`App.jsx`（1,673行 / 103KB）は品質ゲートの基準（5,000行 / 400KB）以内でしたが、
**中核ロジックがテストできない**という実害がありました。画面の組み立てと同じファイルに
入っているため、React を起動しないと1文字も検証できない状態でした。

そこで、見本テキストの解析（文章 → マス目 → ページ）だけを
`src/parseText.js`（React 非依存の純粋関数）へ切り出しました。

| | 前 | 後 |
|---|---:|---:|
| `src/App.jsx` | 1,673行 / 103,670 B | **1,519行 / 95,500 B** |
| `src/parseText.js` | — | 192行 / 9,586 B |

**ロジックは1文字も変更していません。**移動と、定数の import 経路の付け替えだけです。
`useParsedText` は `useMemo` で包むだけの薄いラッパーとして `App.jsx` に残しています
（呼び出し側の書き換えを発生させないため）。

### 「壊していない」ことの証明方法

分割は最も回帰を招きやすい作業のため、**変更前の描画を先に記録**してから作業しました。

代表的な6状態（国語15マス縦 / 算数17マス横 / 原稿用紙400字 / なぞり書き / 穴埋め /
算数22マス横）について、記法をひととおり含む同一テキストを描画し、
`localStorage` を固定して**まったく同じ条件**でスクリーンショットを撮影。
分割後に同じ手順で撮り直して比較しました。

> **結果：6状態すべてが PNG のバイト列レベルで完全一致。**
> 描画されたテキスト構造の比較も全一致。1画素も変わっていません。

### `src/parseText.js` へのテスト（20件）

切り出しの目的だったテストを追加しました。記号の解釈・折り返し・ページ分割・
禁則処理・境界計算などを固定しています。

| 観点 | 件数 |
|---|---:|
| マス目への割り付け・折り返し・ページ分割 | 3 |
| 記号の解釈（【め】【ま】【じ】【終】【赤字】【線】【穴】【枠】【○】日付） | 10 |
| 禁則処理（句読点の合体・行頭送り） | 2 |
| 空入力・空行・位置情報・縦横の違い | 5 |

`mergeNotes` の 9 件とあわせて **合計 29 件**、すべて通過。

## §4：品質ゲートの横展開

正本（`SchoolPlan_Editor/scripts/lib/project-quality.mjs`）を **バイト単位で同一のまま**
移植しました。リポジトリ間で検査内容が分岐しないようにするためです。

```
scripts/check-project.mjs        md5: bb1142758bbe3be19708f0fa74cda07d
scripts/lib/project-quality.mjs  md5: 494844c7b70441dae474dbc65346b47c
```

`quality.config.json` はこのリポジトリの実態（B型・Vite）に合わせて作成し、
`npm run check` として CI に組み込みました。

現在の結果： **0 error / 1 warning**
（warning は `package-lock.json` が 6,618 行という自動生成ファイルに対するもの。
検査を緩めて隠すのではなく、警告として見えるままにしています）

### 正本に対する改善提案（このリポジトリでは実施していません）

`project-quality.mjs` の `TEXT_EXTENSIONS` / `CODE_EXTENSIONS` に **`.jsx` が含まれていません。**
そのため B型（React）リポジトリでは、**最大のファイルである `App.jsx` が検査対象外**になります。
実際、分割前の 1,673 行の `App.jsx` はゲートを素通りしていました。

正本を勝手に書き換えると 57 リポジトリ間で分岐が生じるため、**ここでは変更していません。**
`SchoolPlan_Editor` 側で以下を適用し、各リポジトリへ配り直すことをご検討ください。

```javascript
// project-quality.mjs の先頭
const TEXT_EXTENSIONS = new Set([
  '.css', '.gs', '.html', '.js', '.json', '.jsx', '.md', '.mjs', '.yml', '.yaml'
]);
const CODE_EXTENSIONS = new Set(['.gs', '.html', '.js', '.json', '.jsx', '.mjs', '.yml', '.yaml']);
```

（`RUNTIME_SCRIPT_EXTENSIONS` への `.jsx` 追加も同様に有効です。
JSX は `vm.Script` で構文解析できないため、構文チェック対象には**含めない**でください。
上記2つは行数・バイト数・秘密情報・コンフリクト痕の検査だけに使われるので安全です。）
